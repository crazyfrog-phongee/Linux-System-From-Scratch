...
/* Typedef bool data type or use C99 #include <stdbool.h> */
typedef enum
{
    false,
    true
 ...
 * \note error handling: use assert() to check if '*list' is not NULL at the start of the function.
 */
 ...
 * - If 'insert_copy' is true: use element_copy() to make a copy of 'element' and use the copy in the new list node
 * - If 'insert_copy' is false: insert 'element' in the new list node without taking a copy of 'element' with element_copy()
 * \param list a pointer to the list
 ...
 * \note the first list node has index 0.
 */
