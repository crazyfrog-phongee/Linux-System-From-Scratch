...
dplist_t *dpl_insert_at_index(dplist_t *list, void *element, int index, bool insert_copy)
{
    ...
    //TODO: check with the original src code
    if (index <= 0) /* Thêm vào vị trí đầu */
    {
        if (list->head == NULL) /* Linked List trống */
        {
            list->head = dummy;
            dummy->next = NULL;
        }
        ...
        {
            list->head->prev = dummy; /* Prev của head ban đầu trỏ về node mới */
            dummy->next = list->head; /* Next của node mới trỏ về head ban đầu */
            list->head = dummy;       /* Cập nhật lại head là node mới */
        }
        dummy->prev = NULL; /* Prev của node mới mặc định là NULL */
    }
    else
    {
        dplist_node_t *dummy_old = dpl_get_reference_at_index(list, index);
        assert(dummy_old != NULL);
        if (index >= dpl_size(list)) /* Thêm vào vị trí cuối */
        {
            dummy_old->next = dummy; /* Next của phần tử cuối trỏ về node mới */
            dummy->prev = dummy_old; /* Prev của node mới trỏ về phần tử cuối */
            dummy->next = NULL;      /* Next của node mới mặc định là NULL */
        }
        else /* Thêm vào vị trí xác định */
        {
            dummy->next = dummy_old;       /* Next của node mới trỏ về vị trí node index */
            dummy->prev = dummy_old->prev; /* Prev của node mới trỏ về vị trí node index - 1 (prev của node index)*/
            dummy_old->prev->next = dummy; /* Next của node index - 1 (Next của prev của node index) trỏ về vị trí node mới */
            dummy_old->prev = dummy;       /* Prev của node index trỏ về vị trí node mới */
        }
    }
    if (insert_copy == true)
    {
...
dplist_t *dpl_remove_at_index(dplist_t *list, int index, bool free_element)
{
    ...
    {
        int id = 0;
        while (dummy->next != NULL && id < index)
            dummy = dummy->next, id++;
        if (free_element == true)
...
int dpl_get_index_of_element(dplist_t *list, void *element)
{
    ...
    if ((list->element_compare)(element, dummy->element) == 0)
        return id; // If a member with equalling element is found, returns its index, otherwise returns -1
    return -1;
}
...
dplist_node_t *dpl_get_reference_at_index(dplist_t *list, int index)
{
    ...
    int id = 0;
    while (dummy->next != NULL && id < index)
        dummy = dummy->next, id++; // Goes through the list until the end or the specified location
...
void *dpl_get_element_at_reference(dplist_t *list, dplist_node_t *reference)
{
    ...
    {
        while (dummy->next != NULL)
            dummy = dummy->next; // Gets the last member of the list and returns its element's pointer
        return dummy->element;
    }
    else
    {
        while (dummy->next != NULL && dummy != reference)
            dummy = dummy->next; // Goes through the list until the end or until the reference is found
        if (dummy == reference)
            return dummy->element; // If reference is found, return its element's data
...
dplist_node_t *dpl_get_next_reference(dplist_t *list, dplist_node_t *reference)
{
    ...
    if (dummy == NULL)
        return NULL;    // // Will return NULL if reference is not a member of the list or list is empty
...
dplist_node_t *dpl_get_reference_of_element(dplist_t *list, void *element)
{
    ...
    if ((list->element_compare)(element, dummy->element) == 0)
        return dummy; // If matching member was found, return the found member
...
dplist_t *dpl_insert_at_reference(dplist_t *list, void *element, dplist_node_t *reference, bool insert_copy)
{
    ...
    else if (list->head != NULL && reference != NULL)
    {
        for (dummy = list->head; dummy->next != NULL && dummy != reference; dummy = dummy->next)
            ; // Goes through the list until end is reached or until finds a member pointing to the same dplist_node
        if (dummy->next == NULL && dummy != reference)
            return list; // Returns unmodified list if reference is not an existing member of the list
    }
    else
    {
...
dplist_t *dpl_remove_element(dplist_t *list, void *element, bool free_element)
{
    ...
    if (list->head == NULL)
        return list;
    dplist_node_t *dummy;
    for (dummy = list->head; dummy->next != NULL && (list->element_compare)(dummy->element, element) != 0; dummy = dummy->next)
        ; // Goes through list until end or until finds a member of equal element
    if (dummy->next == NULL && dummy->element != element)
        return list;                                   // No member pointing to 'element' was found in list
    return dpl_remove_node(list, dummy, free_element); // Implementation takes care of freeing
}
...
dplist_t *dpl_remove_node(dplist_t *list, dplist_node_t *dummy, bool free_element)
{
    ...
    if (dummy == list->head) // If element to remove is first in the list
    {
        list->head = dummy->next;
        if (dummy->next != NULL)
            dummy->next->prev = NULL; // If list is larger than 1 element
    }
    else // If element is anywhere else in list
    {
        dummy->prev->next = dummy->next; // Applies to dummy in the middle and at the end of list
        if (dummy->next != NULL)
            dummy->next->prev = dummy->prev; // Applies to dummy not at the end of list
    }
    if (free_element == true)
        (list->element_free)(&(dummy->element)); // If element to free
